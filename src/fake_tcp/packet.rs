...
pub fn build_tcp_packet(
    ...
    let mut buf = BytesMut::zeroed(total_len + ETHERNET_HEADER_LEN);
    ...
    let mut ether = ethernet::MutableEthernetPacket::new(&mut ether_buf).unwrap();
    ether.set_ethertype(ethernet::EtherTypes::Ipv4);
...
pub fn parse_ip_packet(buf: &Bytes) -> Option<(IPPacket, tcp::TcpPacket)> {
    let buf = &buf[ETHERNET_HEADER_LEN..];
    if buf[0] >> 4 == 4 {
